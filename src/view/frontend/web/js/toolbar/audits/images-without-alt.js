...
export default {
  ...
  run(context, active) {
    ...
    const visible = Array.from(document.querySelectorAll("img")).filter(
      (img) => {
        if (!img.offsetParent && getComputedStyle(img).position !== "fixed")
          return false;
    ...
    applyHighlight(warnings, this.key, context, {
      severity: "warning",
      skipBadge: true,
    });
