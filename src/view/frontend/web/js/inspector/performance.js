...
export const performanceMethods = {
  ...
  renderCacheSection(container, cacheData) {
    ...
    const cacheableLabel =
      cacheData.pageCacheable === false
        ? "Block Cacheable (ignored)"
        : "Block Cacheable";
    container.appendChild(
      this.createInfoSection(cacheableLabel, cacheableText, cacheableColor),
    );

    // Cache lifetime (show for all cacheable blocks)
    if (cacheData.cacheable) {
      const lifetimeText =
        cacheData.lifetime === null || cacheData.lifetime === 0
          ? "Unlimited"
          : `${cacheData.lifetime}s`;
      container.appendChild(
        this.createInfoSection("Cache Lifetime", lifetimeText, "#60a5fa"),
      );
    }
    ...
    if (cacheData.key && cacheData.key !== "") {
      container.appendChild(
        this.createInfoSection("Cache Key", cacheData.key, "#a78bfa"),
      );
    }
    ...
    if (cacheData.tags && cacheData.tags.length > 0) {
      ...
      container.appendChild(
        this.createInfoSection("Cache Tags", tagsText, "#22d3ee"),
      );
    }
  },
  ...
  renderBrowserMetricsTab(container, element) {
    ...
    if (this.renderImageOptimizationMetric(container, element))
      hasMetrics = true;
  ...
  renderPageTimingsSection(container) {
    ...
    divider.style.cssText =
      "border-top: 1px solid rgba(255,255,255,0.1); margin: 12px 0 8px; padding-top: 8px; font-size: 10px; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em;";
    divider.textContent = "Global Page-Metrics";
    container.appendChild(divider);

    const dclColor =
      this.pageTimings.domContentLoaded < 1500
        ? "#34d399"
        : this.pageTimings.domContentLoaded < 3000
          ? "#f59e0b"
          : "#ef4444";
    container.appendChild(
      this.createInfoSection(
        "DOM Content Loaded",
        `${this.pageTimings.domContentLoaded} ms`,
        dclColor,
      ),
    );
    ...
    if (this.pageTimings.loadComplete > 0) {
      const loadColor =
        this.pageTimings.loadComplete < 2500
          ? "#34d399"
          : this.pageTimings.loadComplete < 5000
            ? "#f59e0b"
            : "#ef4444";
      container.appendChild(
        this.createInfoSection(
          "Page Load Complete",
          `${this.pageTimings.loadComplete} ms`,
          loadColor,
        ),
      );
    }
  ...
  renderRenderTimeMetric(container, element) {
    ...
    if (blockData && blockData.performance) {
      ...
      container.appendChild(
        this.createInfoSection("PHP Render Time", formattedTime, color),
      );
    ...
    if (this.webVitals.lcp && this.webVitals.lcp.element) {
      ...
      if (isLCP) {
        ...
        container.appendChild(
          this.createInfoSection(
            "LCP Element",
            "✅ This element is critical for LCP!",
            "#ef4444",
          ),
        );
        return true;
  ...
  renderElementTimingMetric(container, element) {
    ...
    if (elementTiming) {
      const timingValue = (
        elementTiming.renderTime || elementTiming.loadTime
      ).toFixed(0);
      const timingColor =
        timingValue < 2500
          ? "#34d399"
      ...
      container.appendChild(
        this.createInfoSection(
          "Element Timing",
          `${timingValue} ms (${elementTiming.identifier})`,
          timingColor,
        ),
      );
      return true;
    ...
    if (imageAnalysis) {
      const modernScore =
        imageAnalysis.totalImages > 0
          ? (
              (imageAnalysis.modernFormats / imageAnalysis.totalImages) *
              100
            ).toFixed(0)
          : 0;
      const modernColor =
        modernScore > 75 ? "#34d399" : modernScore > 25 ? "#f59e0b" : "#ef4444";
      container.appendChild(
        this.createInfoSection(
          "Modern Image Formats",
          `${modernScore}% (${imageAnalysis.modernFormats}/${imageAnalysis.totalImages})`,
          modernColor,
        ),
      );
      ...
      const responsiveScore =
        imageAnalysis.totalImages > 0
          ? (
              (imageAnalysis.hasResponsive / imageAnalysis.totalImages) *
              100
            ).toFixed(0)
          : 0;
      ...
      if (imageAnalysis.oversized > 0) {
        container.appendChild(
          this.createInfoSection(
            "Oversized Images",
            `${imageAnalysis.oversized} oversized`,
            "#ef4444",
          ),
        );
      }
      ...
      if (imageAnalysis.issues.length > 0) {
        ...
        const moreText =
          imageAnalysis.issues.length > 3
            ? ` (+${imageAnalysis.issues.length - 3} more)`
            : "";
        container.appendChild(
          this.createInfoSection(
            "Optimization Tips",
  ...
  renderElementResourceMetrics(container, resourceData) {
    ...
    container.appendChild(
      this.createInfoSection(
        "Element Resources",
        `${resourceData.count} ${resourceLabel} (${sizeText})`,
        "#60a5fa",
      ),
    );
    ...
    images.forEach((img, idx) => {
      ...
      if (img.hasAttribute("srcset") || img.hasAttribute("sizes")) {
        ...
        analysis.issues.push(
          `Image ${idx + 1}: Missing srcset for responsive optimization`,
        );
      }
      ...
      if (img.naturalWidth && img.width) {
        ...
        if (oversizeRatio > 1.5) {
          ...
          analysis.issues.push(
            `Image ${idx + 1}: ${wastedPercent}% oversized (${img.naturalWidth}px served, ${img.width}px displayed)`,
          );
        }
      }
    });
