...
export const tabsMethods = {
  ...
  createTabSystem(data, element) {
    ...
    tabs.forEach((tab) => {
      ...
      button.type = "button";
      button.className =
        "mageforge-tab-button" + (this.activeTab === tab.id ? " active" : "");
  ...
  switchTab(tabId, data, element) {
    ...
    const targetElement =
      element || this.hoveredElement || this.selectedElement;
    if (targetElement) {
  ...
  renderStructureSections(data, container) {
    ...
    container.appendChild(
      this.createInfoSection("Template", data.template, "#60a5fa"),
    );

    // Block section
    container.appendChild(
      this.createInfoSection("Block", data.blockClass, "#a78bfa"),
    );

    // Optional sections
    if (data.blockAlias) {
      container.appendChild(
        this.createInfoSection("Block Name", data.blockAlias, "#34d399"),
      );
    }
    if (data.parentBlock) {
      container.appendChild(
        this.createInfoSection("Parent Block", data.parentBlock, "#fb923c"),
      );
    }
    if (data.viewModel) {
      container.appendChild(
        this.createInfoSection("ViewModel", data.viewModel, "#22d3ee"),
      );
    }
    ...
    if (data.cmsBlockId) {
      container.appendChild(
        this.createInfoSection("CMS Block", data.cmsBlockId, "#f472b6"),
      );
    }

    // Module section
    container.appendChild(
      this.createInfoSection("Module", data.module, "#fbbf24"),
    );
  },
};
