...
export const draggableMethods = {
  ...
  handleDragStart(e) {
    ...
    if (
      ...
      e.target.closest(".mageforge-info-value")
    ) {
      return;
    }
  ...
  handleDrag(e) {
    ...
    newX = Math.max(
      ...
      Math.min(newX, window.scrollX + window.innerWidth - badgeWidth - margin),
    );
    newY = Math.max(
      window.scrollY + margin,
      Math.min(
        newY,
        window.scrollY + window.innerHeight - badgeHeight - margin,
      ),
    );
  ...
  createConnector() {
    ...
    window.addEventListener("scroll", this.connectorScrollHandler, {
      passive: true,
    });
