...
export const vitalsMethods = {
      ...
      if (shift.sources) {
        shift.sources.forEach((source) => {
          if (
            source.node &&
            (source.node === element ||
              element.contains(source.node) ||
              source.node.contains(element))
          ) {
  ...
  getElementTiming(element) {
    if (
      ...
      this.webVitals.elementTimings.length === 0
    ) {
      return null;
    }

    // Check if this element or any child has element timing
    const timing = this.webVitals.elementTimings.find(
      (et) => et.element === element || element.contains(et.element),
    );
  ...
  getBlockMetaData(element) {
    ...
    const hasCacheData =
      ...
      (data.cache.lifetime === null ||
        typeof data.cache.lifetime === "number") &&
