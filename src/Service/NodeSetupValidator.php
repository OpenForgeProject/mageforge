...
class NodeSetupValidator
{
    ...
    private function promptUserForRestoration(array $missing, SymfonyStyle $io): bool
    {
        ...
        foreach ($missing as $item) {
            $suffix = $this->isGeneratedFileOrDirectory($item) ? ' (will be generated by npm install)' : '';
     ...
     * @return bool True if restoration was successful
     */
    private function restoreMissingFiles(string $rootPath, array $missing, SymfonyStyle $io, bool $isVerbose): bool
    {
        if (empty($missing)) {
            return true;
        }
        ...
        if (!$this->fileDriver->isDirectory($basePath)) {
            $io->error(sprintf('Magento base directory not found at: %s', $basePath));
            return false;
        }
    ...
    {
        if (!empty($restored)) {
            $io->success(sprintf('Restored %d file(s): %s', count($restored), implode(', ', $restored)));
        }

        if (!empty($failed)) {
            $io->warning(sprintf('Failed to restore %d file(s): %s', count($failed), implode(', ', $failed)));
        }
    }
